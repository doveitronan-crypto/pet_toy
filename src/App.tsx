...
import { Sparkles, Compass, Heart, User, Dog, MessageSquare, ListCollapse, Star } from 'lucide-react';
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'discover' && (
              ...
              >
                ...
                {/* Curated/Customized directory of toys */}
                <div className="flex flex-col gap-3.5 mt-2">
                  <h3 className="font-extrabold text-sm text-brand-text">精选情报目录</h3>
                  <ToyList
                    toys={CURATED_TOYS}
                    activePet={activePet}
                    isMatched={isMatched}
                    ...
                    isWishlisted={isWishlisted}
                    onToggleWishlist={handleToggleWishlist}
                    onAskGemini={handleAskGeminiDetail}
                  />
                </div>
              </motion.div>
            )}
              ...
              <div className="p-4 border-t border-brand-border flex gap-3">
                <button
                  onClick={() => {
                    ...
                    setDetailToyInModal(null);
                  }}
                  className="flex-1 bg-brand-primary text-white font-bold text-xs py-3 rounded-xl hover:bg-brand-primary/95 shadow-sm"
                >
                  {isWishlisted(detailToyInModal.id) ? '从心愿单中移除' : '加入我的心愿单'}
                </button>
              </div>
            </div>
          </div>
